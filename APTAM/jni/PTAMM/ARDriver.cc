...
namespace PTAMM {
        ...
        :mCamera(cam), mGLWindow(glw), mpMap( &map )
    {
        ...
        mbInitialised = false;
        __android_log_print(ANDROID_LOG_INFO, "ARDriver::ARDriver", "mirFrameSize : %d %d", mirFrameSize.x, mirFrameSize.y);
    }
    ...
    void ARDriver::Init()
    {
        ...
        try {
            CVD::img_load(mLostOverlay, "/sdcard/PTAMM/ARData/Overlays/searching.png");
        }
        catch(CVD::Exceptions::All err) {
            cerr << "Failed to load searching image " << "\"/sdcard/PTAMM/ARData/Overlays/searching.png\"" << ": " << err.what << endl;
        }  
    ...
    void ARDriver::MakeFrameBuffer()
    {
        ...
        // FrameBuffer mnFrameBuffer
        glGenFramebuffersEXT(1, &mnFrameBuffer);
        glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, mnFrameBuffer);
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT,
                GL_TEXTURE_2D, mnFrameBufferTex, 0);            // Bind mnFrameBufferTex
        glFramebufferRenderbufferEXT(GL_FRAMEBUFFER_EXT, GL_DEPTH_ATTACHMENT_EXT,
                GL_RENDERBUFFER_EXT, DepthBuffer);                      // Bind DepthBuffer
    ...
    void ARDriver::Render(Image<Rgb<CVD::byte> > &imFrame, SE3<> se3CfromW, bool bLost)
    {
        ...
        __android_log_print(ANDROID_LOG_INFO, "ARDriver::Render", "imFrame data : %d", (int)(((unsigned char*)(imFrame.data()))[0]));
        __android_log_print(ANDROID_LOG_INFO, "ARDriver::Render", "mirFBSize : %d %d", mirFBSize.x, mirFBSize.y);

        // Upload the image to mnFrameTex
        glBindTexture(GL_TEXTURE_2D, mnFrameTex);
