...
{
    {
        ...
        else
        {
            ...
            if(nToShow >= (int) mvCalibImgs.size())
                nToShow = static_cast<int>(mvCalibImgs.size())-1;
...
void CameraCalibrator::GUICommandHandler(string sCommand, string sParams)  // Called by the callback func..
{
    ...
    {
        int nToShow = (*mgvnShowImage - 1 + 1) % static_cast<int>(mvCalibImgs.size());
        *mgvnShowImage = nToShow + 1;
        return;
    }
    if(sCommand=="CameraCalibrator.SaveCalib")
    {
        ...
        ofstream ofs(GLWindow2::getFDir()+"camera.cfg");
        if(ofs.good())
        {
...
{
    int nViews = static_cast<int>(mvCalibImgs.size());
