...
/// Assembles a position vector and a rotation quaternion from the state
/** it does not set the header */
template<typename stateVector_T, typename StateDefinition_T>
void GenericState_T<stateVector_T, StateDefinition_T>::ToPoseVectors(
    Eigen::Matrix<double, 3, 1>& position, Eigen::Matrix<double, 4, 1> & rotation) {
    
    const Eigen::Matrix<double, 3, 1> & p = Get<StateDefinition_T::p>();
    position[0] = p[0];
    position[1] = p[1];
    position[2] = p[2];
    
    const Eigen::Quaternion<double> & q =  Get<StateDefinition_T::q>();
    rotation[0] = q.w();
    rotation[1] = q.x();
    rotation[2] = q.y();
    rotation[3] = q.z();
